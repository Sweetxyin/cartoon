        ...
        methods: {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {//如果验证通过
                        this.$axios//将更新后的值传到服务端保存
                            .post("/backstage/adminmanage/adminRoleRelationManage/addRolesForAdmins",JSON.stringify(this.adminForm))
            ...
            removeRolesFromAdmins(formName) {//将角色批量从多个账户中删除
                this.$refs[formName].validate((valid) => {
                    if (valid) {//如果验证通过
                        this.$axios//将更新后的值传到服务端保存
                            .delete("/backstage/adminmanage/adminRoleRelationManage/removeRolesFromAdmins",{data: this.adminForm})
                            .then(response => {//获取返回数据
                                let msg=response.data;
                                if (msg.code === 0) {//如果修改成功
                                    this.$message({
