<template>
  <el-dialog title="编辑用户" :visible.sync="dialogFormVisible"  v-loading="loading" width="500px" @opened="openDialogChecked" @closed="closeDailogClean">
        ...
        methods: {
            ...
            openDialogChecked(){//Dialog 打开的回调，检查父组件是否传递了id过来
                if(this.id==''){//如果没有接收到父窗口传递过来的值
                    ...
                    this.loading=true;
                    this.$axios
                        .get("/backstage/usermanage/"+this.id)
                        .then(response => {//获取返回数据/
                            let msg=response.data;
                            if (msg.code === 0) {
                                this.userForm = msg.data;
            ...
            closeDailogClean() {//Dialog 关闭动画结束时的回调。这里是在关闭窗口时清空窗口的编辑信息
                this.$refs["userForm"].resetFields();//重置表单，以免下次打开时还存在，导致让客户看到值变化的过程
            },
            submitForm(formName) {//提交表单事件
                this.$refs[formName].validate((valid) => {
                    if (valid) {//如果验证通过
                        this.$axios//将更新后的值传到服务端保存
                            .put("/backstage/usermanage",JSON.stringify(this.userForm))
                            .then(response => {//获取返回数据
                                let msg=response.data;
